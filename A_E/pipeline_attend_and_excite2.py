    ...
    def _compute_max_attention_per_index(self,
        ...
        for i in indices_to_alter:
            ...
            if sample_strategy == None:
                pass
            elif sample_strategy == 'gw' or 'Gaussian weight':
                # 创建坐标网格
                y, x = torch.meshgrid(torch.linspace(-1, 1, h), torch.linspace(-1, 1, w))
                # 计算到中心的距离
                distance_from_center = torch.sqrt(x.pow(2) + y.pow(2)).to(image.device)
                # 创建高斯权重衰减
                sigma_center = 0.5  # 控制衰减速度
                center_weight = torch.exp(-distance_from_center.pow(2) / (2 * sigma_center ** 2))
                # 应用中心权重
                image = image * center_weight
            elif sample_strategy == 'cc' or 'center_crop':
                length_side = 4 # 默认截取中心1/2区域
                start_h = h//length_side
                end_h = h - h//length_side
                start_w = w//length_side
                end_w = w - w//length_side
                # 截取中心区域
                image = image[start_h:end_h, start_w:end_w]
            elif sample_strategy == 'gc' or 'gravity center':
                # 创建坐标网格
